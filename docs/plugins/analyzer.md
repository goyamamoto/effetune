...
Shows audio input, processing, and output latency. Input and output values are read from browser APIs when available. Output latency relies on `AudioContext.getOutputTimestamp` or the `outputLatency` property and also measures the extra delay introduced when using a `MediaStreamDestination` with an `<audio>` element. When that element lag cannot be measured, the last valid value is reused. All latency values are sampled in microseconds and displayed in milliseconds. Output latency values are averaged over ten seconds to avoid short spikes. Processing latency is sampled by the audio engine every once per second and displayed as a moving average. Values refresh once per second. Latency numbers keep the last successful measurement, and "N/A" is shown until one is available.
